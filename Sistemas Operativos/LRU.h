...
public:
    LRU(int tamanio) : tamanio(tamanio), fallos(0)
    {
    ...
    void accederPagina(int numeroPagina)
    {
        ...
        int lruNumero = ordenUso.front();
        ordenUso.erase(ordenUso.begin());

        for (int i = 0; i < tamanio; ++i)
        {
            if (marcos[i] && marcos[i]->numero == lruNumero)
            {
                cout << "Reemplazando LRU " << lruNumero << " por " << numeroPagina << " en marco [" << i << "]\n";
                delete marcos[i];
                marcos[i] = new Pagina(numeroPagina);
                break;
            }
